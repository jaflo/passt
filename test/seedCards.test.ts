...
describe('seedCards', () => {
    it('should add every combination of the card attributes provided', async () => {
        ...
        assert.equal(await Card.countDocuments({}), shapes.length * fills.length * colors.length * numbers.length);
    })
    ...
    it('should only add the cards that are not already present in the database', async () => {
        ...
        const expectedCount = fullShapes.length * fullFills.length * fullColors.length * fullNumbers.length;
        const actualCount = await Card.countDocuments({});
        const cards = await Card.find({}).sort([['shape'], ['fillStyle'], ['color'], ['number']]);
        for (let i = 0; i < cards.length; ++i) {
            for (let j = i + 1; j < cards.length; ++j) {
                if (cards[i].color === cards[j].color && cards[i].shape === cards[j].shape && cards[i].fillStyle === cards[j].fillStyle && cards[i].number === cards[j].number) {
